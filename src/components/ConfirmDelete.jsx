...
export default function ConfirmDelete({ onConfirm, disabled, variant = "icon", label = "Törlés", confirmLabel = "Biztos?" }) {
  ...
  if (confirming) {
    return (
      <span className="confirm-inline" onClick={stop}>
        <span className="confirm-q">{confirmLabel}</span>
  ...
  if (variant === "full") {
    return (
      <button type="button" className="btn sm danger" disabled={disabled} onClick={(e) => { stop(e); setConfirming(true); }}>{label}</button>
    );
  }
  return (
